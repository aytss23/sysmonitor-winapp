...
from core.thread import SystemMonitorThread
...
class SystemMonitorController():
    ...
    def __init__(self): 
        ...
        self.start_thread() # Thread'i başlat.

    def start_thread(self): 
        self.thread = QThread() # Yeni bir thread oluştur.
        self.thread_model = SystemMonitorThread() # SystemMonitorThread sınıfından bir nesne oluştur

        self.thread_model.moveToThread(self.thread) # thread_model nesnesini thread'e taşı.

        self.thread.started.connect(self.thread_model.start_updating_curr_data) # thread başlatıldığında thread_model'in start_updating_curr_data metodunu çağır.

        # thread_model'in _inst_data_updated sinyalini get_inst_data metoduna bağla.
        self.thread_model._inst_data_updated.connect(self.get_inst_data)
        
        # thread_model'ni _finished sinyalini stop_thread metoduna bağla. 
        self.thread_model._finished.connect(self.thread_model.stop_thread)
